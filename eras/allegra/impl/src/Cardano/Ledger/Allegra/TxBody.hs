...
instance AllegraEraTxBody era => FromCBOR (AllegraTxBodyRaw ma era) where
  fromCBOR =
    decode
      ( SparseKeyed
          "AllegraTxBodyRaw"
...
instance (EraTxOut era, ToCBOR ma, Monoid ma) => ToCBOR (AllegraTxBodyRaw ma era) where
...
bodyFields :: EraTxOut era => Word -> Field (AllegraTxBodyRaw era)
...
emptyAllegraTxBodyRaw :: AllegraTxBodyRaw era
emptyAllegraTxBodyRaw =
  AllegraTxBodyRaw
    empty
    (fromList [])
...
type instance MemoHashIndex AllegraTxBodyRaw = EraIndependentTxBody
...
instance MAClass ma c => EraTxBody (AllegraEra c) where
...
instance MAClass ma c => ShelleyEraTxBody (AllegraEra c) where
...
instance MAClass ma c => AllegraEraTxBody (AllegraEra c) where
  ...
  mintTxBodyL =
    lensMemoRawType atbrMint $ \txBodyRaw mint -> txBodyRaw {atbrMint = mint}
  {-# INLINEABLE mintTxBodyL #-}

  mintValueTxBodyF = to (promoteMultiAsset (Proxy @ma) . atbrMint . getMemoRawType)
  {-# INLINEABLE mintValueTxBodyF #-}

  mintedTxBodyF = to (getScriptHash (Proxy @ma) . atbrMint . getMemoRawType)
