    ...
    DCertPool (RegPool poolParam) -> do
      ...
      if eval (hk ∉ dom stpools)
        then do
          ...
          -- The deposit does not change. One pays the deposit just once. Only it is fully retired
          -- (i.e. it's deposit has been refunded, and it has been removed from the registered pools).
          -- does it need to pay a new deposit (at the current current deposit amount). But of course,
